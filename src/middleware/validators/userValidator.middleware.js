...
const validator = {
  validate: (method) => {
    switch (method) {
      ...
      case endpoint.getUserById: {
        return [
          param("uid").exists().isNumeric().withMessage("Must be a number"),
        ];
      }
      case endpoint.getUserByuserName: {
        return [
          param("username")
            .exists()
            .isString()
            .withMessage("username is required")
            .isLength({ min: 3 })
            .withMessage("Must be string and at least 3 chars long"),
        ];
      }
    }
  },
